...
public class Consultar {
	...
	public void consultar() {
		...
		System.out.println("\npessoa relacionada ao numero 88883333 ");
		q = manager.query();
		q.constrain(Pessoa.class);
		...
		q.descend("numero").constrain("88883333");
		pessoas = q.descend("pessoa").execute(); 	//alteracao no resultado
		for(Pessoa  p : pessoas)
			System.out.println(p);

		System.out.println("\ntelefones da operadora 88");
		q = manager.query();
		q.constrain(Telefone.class);
		q.descend("numero").constrain("88").startsWith(true);
		...
		q = manager.query();
		q.constrain(Telefone.class);
		q.descend("pessoa").descend("nome").constrain("ma").contains();
		telefones = q.execute(); 
		for(Telefone t : telefones)
			System.out.println(t);
...
class Filtro1  implements Evaluation {
	public void evaluate(Candidate candidate) {
		...
		boolean resultado; 
		resultado=false;
		if (t.getPessoa().getNome().contains("ma"))	//maria
			resultado = true	;
